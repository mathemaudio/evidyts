...
export class ResultReporter {
	...
	private static readonly RULE_DESCRIPTIONS: Record<RuleCode, string> = {
		...
		"line-too-wide": `No line may exceed ${BreadthRuleLimits.getConfig().lineWidth.hardMax} characters. Split the statement instead of packing it`,
		"line-width-distribution": `Files must keep a readable line-width mix: ${BreadthRuleLimits.formatLineWidthSummary(BreadthRuleLimits.getConfig())}`,
		"multiple-statements-per-line": "Each line may open only one statement; move members to another file instead of packing lines",
